...
import { Upload, Music, Edit2, Trash2, Save, X, Plus, Search, AlertCircle, CheckCircle, Loader, Eye, EyeOff } from 'lucide-react';
...
const MusicAdmin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        showMessage('success', editingSong ? 'Canción actualizada' : 'Canción subida exitosamente');
        resetForm();
        fetchSongs();
      } else {
        const errorData = await response.json();
        showMessage('error', errorData.message || 'Error al procesar la canción');
      }
    } catch (error) {
      showMessage('error', 'Error de conexión con el servidor');
    } finally {
  ...
  const handleDelete = async (songId) => {
    ...
    try {
      const response = await fetch(`https://singravedad-back.vercel.app/api/music/${songId}`, {
        method: 'DELETE'
      });
          ...
          {isLoading ? (
            ...
            <div className="empty-state">
              ...
              <p className="empty-subtitle">
                {searchTerm ? 'Intenta con otros términos de búsqueda' : 'Comienza subiendo tu primera canción'}
              </p>
              {!searchTerm && (
                <button
                  onClick={() => setShowUploadForm(true)}
